...
class MatMulAddPattern(Pattern):
    ...
    def match(self, node: ONNXNode, graph: ONNXGraph) -> Optional[List[ONNXNode]]:
        ...
        matmul_input0, matmul_input1 = graph.get_initializer_by_name(matmul_inputs[0]), \
                                        graph.get_initializer_by_name(matmul_inputs[1]) 
        
        if (matmul_input0 is not None and matmul_input1 is not None) \
            or (matmul_input0 is None and matmul_input1 is None):
            return None 
        matmul_input_constant, matmul_input_variable_name = (matmul_input0, matmul_inputs[1]) if matmul_input0 is not None else (matmul_input1, matmul_inputs[0])
