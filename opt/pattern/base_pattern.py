...
class Pattern(ABC):
    ...
    def register_pattern(cls, pattern: PatternType):
        if not isinstance(pattern, cls):
            raise TypeError(
                f"The registerd pattern must be {cls.__name__}, input pattern type:{type(pattern)}"
            )
...
class MatchResult:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"MatchResult(pattern={self.pattern.name}, nodes={[n.id for n in self.matched_nodes]})"
        )
