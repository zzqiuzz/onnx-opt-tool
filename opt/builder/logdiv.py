...
def replace_log_div(self, match_result: MatchResult):
    ...
    log_0_output = gs.Variable(
        name=f"{log_node.name}_0_out", dtype=log_output.dtype, shape=log_output.shape
    )
    log_1_output = gs.Variable(
        name=f"{log_node.name}_1_out", dtype=log_output.dtype, shape=log_output.shape
    )
    log_node_0 = self.layer(
        op="Log", inputs=[div_input_0_tensor], outputs=[log_0_output], name=f"{log_node.name}_0"
    )

    log_node_1 = self.layer(
        op="Log", inputs=[div_input_1_tensor], outputs=[log_1_output], name=f"{log_node.name}_1"
    )

    div_node = self.layer(
        op="Sub", inputs=[log_0_output, log_1_output], outputs=[log_output], name=f"{div_node.name}"
    )
