...
def infer_model_and_save_outputs(
    ...
    if input_data is None:
        ...
        for input_name, shape in input_shape.items():
            ...
            logger.info(
                f"Generated random input: {input_name}, shape: {shape}, dtype: {input_dtype}"
            )
    else:
        for input_name, data in input_data.items():
            logger.info(
                f"Using custom input: {input_name}, shape: {data.shape}, dtype: {data.dtype}"
            )
    ...
    if dump_data:
        ...
        for name, tensor in output_dict.items():
            ...
            logger.info(
                f"  Saved {name}: shape={tensor.shape}, dtype={tensor.dtype} -> {save_path}"
            )
...
def insert_op_output(
    model_path: str, dump_model_path: str, op_type=None, insert_node_names=None
) -> list:
