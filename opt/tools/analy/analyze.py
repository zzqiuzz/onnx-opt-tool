...
def analyze(
    ...
    df = pd.DataFrame(
        rows, columns=["op_name", "float_shape", "quant_shape", "mse", "cosine", "note"]
    )
    ...
    if show:
        display_df = df.copy()
        display_df["mse"] = display_df["mse"].apply(
            lambda x: f"{x:.6e}" if (isinstance(x, (int, float)) and not math.isnan(x)) else str(x)
        )
        display_df["cosine"] = display_df["cosine"].apply(
            lambda x: f"{x:.6f}" if (isinstance(x, (int, float)) and not math.isnan(x)) else str(x)
        )
        print(
            tabulate(
