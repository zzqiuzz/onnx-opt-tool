...
def convert_inputs_to_constants(
    ...
    for input_name, const_value in input_constants.items():
        const_node = gs.Constant(name=f"constant_{input_name}", values=const_value)
    ...
    # 从输入中移除已转换的
    graph.inputs = [inp for inp in graph.inputs if inp.name not in input_constants.keys()]
...
if __name__ == "__main__":
    ...
    OUTPUT_ONNX = (
        "/home/uto/workspace/my/onnx-opt-tool/onnx/online_const_ogs.onnx"  # 修改后模型路径
    )
    ...
    convert_inputs_to_constants(INPUT_ONNX, OUTPUT_ONNX, CONST_MAPPING, unused_output)
